    ...
    <div class="invoiceContent">
      <div class="invoice-header">
        ...
        <div>
          <h2>Hóa Đơn</h2>
          <p>
            Mã vận đơn: <span id="tracking-number">{{ item.id }}</span>
          </p>
          <p>
            Mã đơn hàng: <span id="order-number">{{ item.orderId }}</span>
          </p>
        </div>
      </div>
      <div class="row">
        <div class="col-md-6">
          <div class="invoice-address">
            ...
            <p>
              số 1 25/4/23 đường phú minh, Phường Minh Khai, Quận Bắc Từ Liêm,
              Hà Nội
            </p>
          </div>
        </div>
      ...
      <div class="invoice-total">
        <h4>
          Tổng tiền: <span id="total-amount">{{ total }}đ</span>
        </h4>
      </div>
    </div>
  ...
  computed: {
    total() {
      return this.order_details
        .reduce((total, item) => total + item.productPrice * item.qty, 0)
        .toLocaleString();
    },
  },
  methods: {
