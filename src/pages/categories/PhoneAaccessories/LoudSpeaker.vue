...
<div class="container" v-loading="this.$store.state.isLoading">
      <li v-for="item in loudspeaker" :key="item.id" > 
          <div class="card">
              <div class="imgBx">
...
export default {
  ...
  methods: {
    getAll() {
      axios.get(`http://localhost:8181/api/products/listproductbycate?cate=${(this.cateId = 24)}`)
        .then((res) => {
          if (res.data != null) {
            this.loudspeaker = res.data;
    ...
    addcart(item) {
      if(this.show){
      axios.post(`http://localhost:8181/api/cart/insertcart`, {
